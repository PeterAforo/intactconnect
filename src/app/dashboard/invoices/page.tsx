...
import { FileText, Plus, X, Loader2, Send, Clock, CheckCircle, XCircle, Trash2 } from "lucide-react";
...
export default function InvoicesPage() {
  ...
  const handleCreate = async (sendNow: boolean) => {
    ...
    setClientId(""); setTax("0"); setDueDate(""); setNotes("");
    fetchData();
  };
    ...
    <div className="p-4 lg:p-8">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-text-muted">
                  {inv.client?.name || "No client"} &bull; {inv.items.length} item{inv.items.length !== 1 ? "s" : ""}
      ...
      {showCreate && (
        <>
          ...
          <div className="fixed inset-2 sm:inset-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-[600px] sm:max-h-[85vh] bg-white rounded-2xl shadow-2xl z-50 overflow-y-auto">
            <div className="p-6">
              ...
              <div className="space-y-4">
                ...
                {/* Items */}
                <div>
                  <label className="text-sm font-medium text-text block mb-2">Items</label>
                  <div className="space-y-2">
                    {items.map((item, idx) => (
                      <div key={idx} className="flex gap-2 items-start">
                        ...
                        <Input type="number" min="1" value={item.quantity} onChange={(e) => updateItem(idx, "quantity", parseInt(e.target.value) || 1)} className="w-16" />
                        <Input type="number" min="0" step="0.01" value={item.unitPrice} onChange={(e) => updateItem(idx, "unitPrice", parseFloat(e.target.value) || 0)} className="w-24" placeholder="Price" />
                        {items.length > 1 && (
                          <button onClick={() => removeItem(idx)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 className="w-4 h-4" /></button>
                        )}
                      </div>
                    ))}
                    <button onClick={addItem} className="text-primary text-sm hover:underline">+ Add item</button>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-medium text-text block mb-1">Tax (GH₵)</label>
                    <Input type="number" min="0" step="0.01" value={tax} onChange={(e) => setTax(e.target.value)} />
                  </div>
                  <div>
                    <label className="text-sm font-medium text-text block mb-1">Due Date</label>
                ...
                <div className="bg-surface rounded-lg p-3 space-y-1">
                  <div className="flex justify-between text-sm"><span className="text-text-muted">Subtotal</span><span className="text-text">{formatPrice(subtotal)}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-text-muted">Tax</span><span className="text-text">{formatPrice(parseFloat(tax || "0"))}</span></div>
